...
class Funciones {
    ...
    public function convertir_fecha($fecha)
	{
		...
		return $dias[date('w',strtotime($fecha))]." ".date('d',strtotime($fecha))." de ".$meses[date('n',strtotime($fecha))-1]. " del ".date('Y',strtotime($fecha))." a las ".date('H',strtotime($fecha))." Hrs con ".date('i',strtotime($fecha))." Min" ;
		
	}
	
	public function convertirFecha_YM($fecha)
	{
		$dias = array("Domingo","Lunes","Martes","Miercoles","Jueves","Viernes","Sábado");
		$meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
 
		return $meses[date('n',strtotime($fecha))-1]. " del ".date('Y',strtotime($fecha)) ;
