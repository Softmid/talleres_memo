...
<article class="imprecionOrden clear">
	...
	<section class="categoriaImpresion">
			<?php
				$subTotal = 0;
				$Total = 0;
				foreach($categorias->result() as $categoria)
				{
					echo '<aside class="categoria">';
					echo '<h2>'.$categoria->nombre.':</h2>';
					echo '<ul>';
					foreach ($rel->result() as $data) 
					{
						if($categoria->idCategoria == $data->idCategoria)
						{
							echo '<li>'.$data->concepto.' <span>$ '.number_format($data->monto, 2).'</span></li>';
							$subTotal += $data->monto;
						}
					}
					echo '</ul>';
					echo '<aside class="subtotal">Sub-Total <span>$ '.number_format($subTotal, 2).'</span></aside>';
					echo '</aside>';
					$Total += $subTotal;
					$subTotal = 0;
				}
